...
const populateAdminRounds = async () => {
	...
	let rounds = await db.collection('rounds_2023');
...
const displayAdminRound = async (roundId) => {
	...
	let round = await db.collection('rounds_2023').doc(roundId.toString());
...
const startRound = async (started, round) => {
	await db.collection('rounds_2023').doc(round.toString()).update({
...
const displayRoundMatchupForAdmin = async (round) => {
	...
	let matchups = await db.collection('rounds_2023').doc(round).collection('matchups');
...
const fetchUser = async (userName) => {
	let user = await db.collection('golfers_2023').doc(userName).get();
	return user.data();
}

const removeGroupingFromRound = async (id, round) => {
	await db.collection('rounds_2023').doc(round.toString()).collection('matchups').doc(id).delete();
...
const addMatchup = async () => {
	...
	let round = await db.collection('rounds_2023').doc(currentRoundForAddingMatchup.toString());
	let snapshot = await round.get();
	...
	let matchups = await db.collection('rounds_2023').doc(currentRoundForAddingMatchup.toString()).collection('matchups')
...
const generateMatchups = async (r) => {
	...
	let matchups = db.collection('rounds_2023').doc(r.toString()).collection('matchups');
