...
const fetchDataFromFirebase = async () => {
	roundsData = await db.collection('rounds_2023').get();
	golfersData = await db.collection('golfers_2023').get();
}
...
const setupScorecard = async (matchups, round) => {
	...
	let roundData = await db.collection('rounds_2023').doc(round.toString()).get();
...
const determineMatchupScoreboard = async (matchup) => {
	...
	await db.collection('rounds_2023').doc(matchup.round.toString()).collection('matchups').doc(matchup.id).update({scoreboard: matchupScoreboard});
...
const loadScorecard = async (round) => {
	...
	let matchups = [];
	matchups = await db.collection('rounds_2023').doc(round.toString()).collection('matchups').get();
...
const submitCurrentScores = async () => {
	...
	db.collection('rounds_2023').doc(roundNumber.toString()).collection('matchups').doc(id).update(updateObject);
